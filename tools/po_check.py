#!/usr/bin/env python3
"""
PO file inspection tool for PBPicat.
...
def load_catalog(lang: str):
    po_path = LOCALE_DIR / lang / "LC_MESSAGES" / "pbpicat.po"
...
def print_stats(langs):
    header = f"{'lang':<8} {'total':>6} {'transl':>8} {'missing':>8} {'shortcuts':>10} {'fuzzy':>6}"
    print(header)
    print("-" * len(header))
    for lang in langs:
        s = stats_for(lang)
        ...
        print(
            f"{s['lang']:<8} {s['total']:>6} {s['translated']:>8} {miss:>8} {s['shortcut_empty']:>10} {s['fuzzy']:>6}"
        )
    ...
    for lang in langs:
        ...
        for msg in cat:
            if not msg.id:
                continue
            msgid = msg.id if isinstance(msg.id, str) else msg.id[0]
            msgstr = msg.string if isinstance(msg.string, str) else (msg.string or ("",))[0]
...
def main():
    ...
    parser.add_argument("--lang", metavar="LANG", help="restrict to one language (e.g. fr)")
    ...
    parser.add_argument("--msgstr", action="store_true", help="also search in msgstr (with --search)")
    parser.add_argument(
        "--diff",
        nargs=2,
