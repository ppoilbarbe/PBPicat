...
- **`piexif` dropped as a runtime dependency** — `image_ops.set_exif_orientation()` (used to strip/set the EXIF Orientation tag after JPEG rotation) now uses `pyexiv2` instead, matching the reader added for the metadata panel. This also removes `_sanitize_exif_for_dump()`, a piexif-specific workaround for camera firmware storing Undefined-typed EXIF tags as plain integers, no longer needed since `pyexiv2`/libexiv2 handles this internally. `piexif` remains a dev/test-only dependency, used independently in the test suite to build and verify EXIF fixtures.
