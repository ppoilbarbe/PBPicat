...
- **Cursor pixmap**: `drag.setDragCursor(get_icon("duplicate", "edit-copy").pixmap(24, 24), Qt.CopyAction)` before `exec()` — several window managers/compositors don't render Qt's built-in per-action drag cursors distinctly (or at all), so without an explicit pixmap the cursor can look identical for Copy and Move regardless of the negotiated action. Move keeps Qt's own default drag cursor; only Copy is overridden, since it just needs to visibly differ from it.
